 ...
 */
/* global define, $, BEME*/
define('minpage/search', ['comm/utils', 'comm/url', 'comm/request','comm/clickevent', 'helper/app', 'helper/temphtml', 'require', 'exports'],
    function(mUtils, mUrl, mRequest,mClick, mApp, mTemp, require, exports){
        "use strict";
		var inputString = '',
		_songList = {},
		_isJump_album = false,
		_isJump_station = false,
		_songData={},
		...
		var _renderSongInfo = function(jdata,name){
			...
			$.each(jdata.songs, function(i, info){
			 ...
			 _songData = mApp.getSongJson(jdata.songs,'','宝宝爱听，贝美时光说');
			...
			$box.show();
			 $('#song-box').off('click');
			 if(isNew){
				$('#song-box').find('.item-play img').attr('src','./static/images/feiji.png');
				$('#song-box').on('click', '.item .item-a', function(){
					 mClick.audition_new(_songData,this);
				});
			...
			mRequest.getRequest({
                ...
                successfn : function(jdata){
                    ...
                    if(jdata.code===0){
						...
						if(isNew){
							$('#random_song').find('.item-play img').attr('src','./static/images/feiji.png');
						}
						
						$('#random_song').off('click');
						
						if(isNew){
							 _songData = mApp.getSongJson(jdata.data,'','宝宝爱听，贝美时光说');
